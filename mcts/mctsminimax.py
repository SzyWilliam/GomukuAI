    ...
    def constructTree(self, currentBoard, player, nodePosition, maxDepth=10, currentDepth=0):
        ...
        for expand_node in expand_nodes:
            if currentDepth >= maxDepth or expand_node.value > 3000:
                # switch to mcts scorer at each leaf node 
                mcts_val = MCTSScorer.evaluate(expand_node.newboard,expand_node.position[0],expand_node.position[1],player)
                # logDebug("mctsvalue = {}".format(mcts_val))
                successors.append(Node(player=3-player, isLeaf=True, value=mcts_val, position=expand_node.position))
